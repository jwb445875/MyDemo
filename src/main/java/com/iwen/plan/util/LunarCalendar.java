...
public class LunarCalendar {
    ...
    public static Date getNewYear(int year){
        int[] d=lunarToSolar(year,1,1,false);
        ...
        sb.append(d[2]);
        return DateUtils.formStrToDate(sb.toString(),"yyyyMMdd");
    }
     ...
     * @return
     */
    public static String getNewYears(Date start,Date end){
        ...
        int sy= DateUtils.getYear(start);
        int ey= DateUtils.getYear(end);
        StringBuilder sb=new StringBuilder();
        Date y;
        for(int i=sy;i<=ey;i++){
            y=getNewYear(i);
            if(y.getTime()<=end.getTime()&&y.getTime()>=start.getTime()){
                sb.append(DateUtils.formDateToStr(y)).append(Contants.COMMA_SEPARATOR);
            }
        }
        if(sb.length()<5){
            return null;
    ...
    public static void main(String[] args) {
        System.out.println(DateUtils.dateTimeToStr(getNewYear(2020)));
    }
}
